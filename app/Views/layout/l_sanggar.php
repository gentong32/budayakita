...
$(document).ready( function () {
    ...
    var data = [];
    <?php foreach ($daftarsanggar as $key => $value) :
      $namalembaga = trim($value->nama,'"');
      $namalembaga = trim($namalembaga,'\'"');
      $namalembaga = str_replace("\"","'",$namalembaga);

      ?>
      data.push([<?=$key + 1?>, "<a href=\"<?=site_url('sanggar/objek/').$value->lembaga_id?>\"><?=$namalembaga;?></a>", "<?=$value->fokus_bidang?>","<?=$value->propinsi?>"]);
