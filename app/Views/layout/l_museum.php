  ...
  <div class="site-wrapper overflow-hidden position-relative">
    ...
    <div class="no-bottom no-top">
      <div class="content">
            ...
            <!-- section begin -->
            <div class="hero-area-l-19" style="background-image:url(<?=base_url()?>/template/image/topimages/bg-wbtb.jpg); top;">
              <div class="col-md-12 text-center" data-aos="fade-left" data-aos-duration="800" data-aos-once="true">
                <div class="content">
            <h1>Kategori Lembaga</h1>
                </div>
          </div>
            </div>
        </div>
...
<div style="margin-bottom:10px;">JENIS LEMBAGA : 
      <select class="combobox1" id="ijenis" name="ijenis">
          <option <?php
            if ($idjenis=="all") echo "selected" 
          ?> value="all">-Semua Jenis-</option>
          <?php
            foreach ($jenislembaga as $row)
            {
              $terpilih = "";
              if ($idjenis==$row->jenis_lk_id) 
                $terpilih = "selected";
              if ($row->jenis_lk_id==6)
              echo "<option ".$terpilih." value=\"".$row->jenis_lk_id."\">Organisasi Penghayat Kepercayaan</option>";
              else
              echo "<option ".$terpilih." value=\"".$row->jenis_lk_id."\">".$row->nama_lk."</option>";
            }
          ?>
      </select>
</div>
...
$(document).on('change', '#ijenis', function () {
        window.open('<?=site_url()."lembaga?jenis=";?>'+$('#ijenis').val(),'_self');
    });
...
$(document).ready( function () {
    ...
    var data = [];
    <?php $nomor=0;?>
    <?php foreach ($daftarmuseum as $key => $value) :
        $nomor++;
        $namalembaga = str_replace('"', "'",$value->nama);
        // if ($nomor==1300)
        // break;
      ?>
      

      data.push([<?=$key + 1?>, "<a href=\"<?=site_url('lembaga/objek/').$value->kode_pengelolaan?>\"><?=$value->kode_pengelolaan?></a>", "<?=str_replace('"', "'",$value->nama)?>", "<?=$value->nama_lk?>","<?=$value->propinsi?>"]);
