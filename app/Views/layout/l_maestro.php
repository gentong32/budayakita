...
$(document).ready( function () {
    ...
    var data = [];
    <?php foreach ($daftarmaestro as $key => $value) :
      $sex = "Laki-laki";
      if (trim($value->jenis_kelamin)=="P")
      $sex = "Perempuan"; 
      ?>
      data.push([<?=$key + 1?>, "<a href=\"<?=site_url('maestro/objek/').$value->maestro_id?>\"><?=$value->nama?></a>","<?=$sex?>","<?=$value->propinsi?>"]);
