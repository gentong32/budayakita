      ...
      <div class="content">
            ...
            <div class="hero-area-l-19" style="background-image:url(<?=base_url()?>/template/image/topimages/bg-wbtb.jpg); top;">
              <div class="col-md-12 text-center" data-aos="fade-left" data-aos-duration="800" data-aos-once="true">
                <div class="content">
            <h1>Kategori Warisan Budaya Takbenda</h1>
                </div>
          </div>
            </div>
        </div>
...
<div>
  Kategori: <select style="width:200px;height:25px;" id="ikategori" name="ikategori">
                            ...
                            echo "selected";?> value="<?=$value->jenis_id?>"><?=$value->jenis_wbtb?></option>
                            <?php endforeach;?>
                        </select>
  <br>
  <br>
...
$(document).ready( function () {
  ...
  $("#ikategori").change(function () {
    if ($('#ikategori').val()==0)
    window.open('<?=base_url()?>/wbtb', '_self');
    else
    window.open('<?=base_url()?>/wbtb?kategori='+$('#ikategori').val(), '_self');
  });

    var data = [];
    <?php foreach ($daftarwbtb as $key => $value) :?>
      data.push([<?=$key + 1?>, "<a href=\"<?=site_url('wbtb/objek/').$value->kode_pengelolaan?>\"><?=$value->kode_pengelolaan?></a>", "<?=$value->nama?>", "<?=$value->jenis_wbtb?>","<?=$value->propinsi?>"]);
